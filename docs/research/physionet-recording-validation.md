...
The completed [manufacturer-document follow-up](physionet-manufacturer-output-followup.md) supports the mechanism of a shared Simulink output grid, but supplies no release-specific wiring, units, or conversion settings. Historical manufacturer material names a 50 Hz optical configuration while current specifications say 10 Hz; neither establishes the native rate here. The remaining unblock is the original acquisition/export configuration or equivalent release-specific clarification.
